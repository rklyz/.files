...
function send_notification {
    ...
    sh ~/.config/scripts/notify-send.sh -i notification-audio-volume-high "$volume%"" ""$bar" -t 2000 -h string:synchronous:"$bar" --replace=555
...
case $1 in
	...
	if is_mute ; then
	   sh ~/.config/scripts/notify-send.sh -i notification-audio-volume-muted "Muted"" ""─────────x─────────" --replace=555 -t 2000
   	else
	    send_notification
	fi
	;;
